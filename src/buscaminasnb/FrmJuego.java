...
public class FrmJuego extends javax.swing.JFrame {
    ...
    void descargarControles() {
        if(botonesTablero != null) {
            for (int i = 0; i < botonesTablero.length; i++) for (int j = 0; j < botonesTablero[i].length; j++) {
                    if(botonesTablero[i][j] != null) getContentPane().remove(botonesTablero[i][j]);
            }
        }
    }
    private void crearTableroBuscaminas() {
        ...
        tableroBuscaminas.setEventoPartidaPerdida(new Consumer<List<Casilla>>() {
            @Override public void accept(List<Casilla> t) {
                for (Casilla casillaConMina : t) botonesTablero[casillaConMina.getPostFila()][casillaConMina.getPosColumna()].setText("*");
                JOptionPane.showMessageDialog(null, "Has perdido el juego!", "Fin del juego", JOptionPane.INFORMATION_MESSAGE);
        ...
        tableroBuscaminas.setEventoPartidaGanada(new Consumer<List<Casilla>>() {
            @Override public void accept(List<Casilla> t) {
                for (Casilla casillaConMina : t) botonesTablero[casillaConMina.getPostFila()][casillaConMina.getPosColumna()].setText(":)");
                JOptionPane.showMessageDialog(null, "Has ganado el juego!", "Fin del juego", JOptionPane.INFORMATION_MESSAGE);
                juegoNuevo(); // Reinicia el juego
            }
        });
        tableroBuscaminas.setEventoCasillaAbierta(new Consumer<Casilla>() {
            @Override public void accept(Casilla t) {
                ...
                botonesTablero[t.getPostFila()][t.getPosColumna()].setText(t.getNumMinasAlrededor() == 0 ? "" : t.getNumMinasAlrededor() + "");
            }
        });
    ...
    private void cargarControles() {
        ...
        for (int i = 0; i < botonesTablero.length; i++) {
            for (int j = 0; j < botonesTablero[i].length; j++) {
                ...
                if (i == 0 && j == 0) botonesTablero[i][j].setBounds(posXReferencia, posYReferencia, anchoControl, altoControl);
                else if (i == 0 && j != 0) botonesTablero[i][j].setBounds(botonesTablero[i][j - 1].getX() + botonesTablero[i][j - 1].getWidth(),
                        posYReferencia, anchoControl, altoControl
                );
                else botonesTablero[i][j].setBounds(botonesTablero[i - 1][j].getX(),
                            botonesTablero[i - 1][j].getY() + botonesTablero[i - 1][j].getHeight(),
                            anchoControl, altoControl
                    );
                botonesTablero[i][j].addActionListener(new ActionListener() {
                    @Override public void actionPerformed(ActionEvent e) {btnClick(e); }
                });
                getContentPane().add(botonesTablero[i][j]);
            }
        }
        this.setSize(botonesTablero[numFilas - 1][numColumnas - 1].getX() + botonesTablero [numFilas - 1][numColumnas - 1].getWidth() + 30,
                botonesTablero[numFilas - 1][numColumnas - 1].getY() + botonesTablero [numFilas - 1][numColumnas - 1].getHeight() + 70
        );
    }
